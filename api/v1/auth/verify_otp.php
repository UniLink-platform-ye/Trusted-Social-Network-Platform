...
// تفعيل الحساب (للتسجيل الجديد) وتحديث last_login
db()->prepare('UPDATE users SET is_verified = 1, last_login = NOW() WHERE user_id = :id')->execute([':id' => $userId]);
$user = fetch_user_by_id($userId); // إعادة جلب is_verified المحدّث
